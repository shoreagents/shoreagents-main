...
export default function TestContentTrackingNewStructure() {
  ...
  const testDirectInsert = async () => {
    ...
    try {
      ...
      // Test direct insert with new structure
      const { data, error } = await supabase
        ...
        .insert([{
          ...
          content_id: 'test-direct-insert-new',
          content_title: 'Test Direct Insert New Structure',
          page_section: 'main',
      ...
      if (error) {
        addTestResult('Direct Insert (New Structure)', `Error: ${error.message}`, false);
      } else {
        addTestResult('Direct Insert (New Structure)', `Success: ${JSON.stringify(data)}`, true);
      }
    } catch (error) {
      addTestResult('Direct Insert (New Structure)', `Exception: ${error}`, false);
    } finally {
      setLoading(false);
    }
  };
  ...
  const testContentTrackingService = async () => {
    setLoading(true);
    try {
      console.log('🧪 Testing content tracking service with new structure...');
      
      const result = await contentTracker.trackPageView(
        'test_page',
        'test-service-new-structure',
        {
          user_id: null, // Let the service determine user_id
          content_title: 'Test Service New Structure',
          page_section: 'main'
        }
      );
  ...
  const testInteractionTracking = async () => {
    ...
    try {
      ...
      const result = await contentTracker.trackInteraction(
        'test_page',
        'test-interaction-new-structure',
        {
          user_id: null,
          content_title: 'Test Interaction New Structure',
          page_section: 'main'
        }
      );
  ...
  const testFormSubmissionTracking = async () => {
    ...
    try {
      ...
      const result = await contentTracker.trackFormSubmission(
        'test_page',
        'test-form-new-structure',
        {
          user_id: null,
          content_title: 'Test Form New Structure',
          page_section: 'main'
        }
      );
  ...
  const checkRecentData = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('content_views')
        .select('id, user_id, content_type, content_id, interaction_type, activity_count, form_submissions, scroll_depth, viewed_at')
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <h1 className="text-3xl font-bold">Content Tracking New Structure Test</h1>
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Button onClick={testDirectInsert} disabled={loading}>
          Test Direct Insert (New Structure)
        </Button>
