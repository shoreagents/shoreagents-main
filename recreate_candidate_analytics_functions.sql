-- Drop existing functions first, then recreate them with correct return types
-- This fixes the "cannot change return type" error

-- Drop existing functions if they exist
DROP FUNCTION IF EXISTS get_candidate_analytics(VARCHAR(255));
DROP FUNCTION IF EXISTS get_user_viewing_history(VARCHAR(255), INTEGER);
DROP FUNCTION IF EXISTS get_top_viewed_candidates(INTEGER, INTEGER);
...
SELECT 'Analytics functions recreated successfully' as status;
