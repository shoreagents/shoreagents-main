...
-- 1. Check existing functions that might conflict
SELECT 'Checking existing functions that might conflict...' as step;
SELECT 
    routine_name,
    routine_type,
...
-- 2. Drop ALL functions that might conflict
SELECT 'Dropping all potentially conflicting functions...' as step;

-- Drop simple_get_analytics specifically
DROP FUNCTION IF EXISTS simple_get_analytics(character varying) CASCADE;
DROP FUNCTION IF EXISTS simple_get_analytics(VARCHAR) CASCADE;
DROP FUNCTION IF EXISTS simple_get_analytics(VARCHAR(255)) CASCADE;

-- Drop increment_candidate_activity specifically
...
DROP FUNCTION IF EXISTS increment_candidate_activity(VARCHAR, VARCHAR, VARCHAR, VARCHAR) CASCADE;
DROP FUNCTION IF EXISTS increment_candidate_activity(VARCHAR(255), VARCHAR(255), VARCHAR(255), VARCHAR(50)) CASCADE;

-- Drop simple_get_anonymous_user specifically
DROP FUNCTION IF EXISTS simple_get_anonymous_user() CASCADE;
DROP FUNCTION IF EXISTS simple_get_anonymous_user(character varying) CASCADE;
DROP FUNCTION IF EXISTS simple_get_anonymous_user(VARCHAR) CASCADE;
DROP FUNCTION IF EXISTS simple_get_anonymous_user(VARCHAR(255)) CASCADE;

-- Drop simple_get_authenticated_user specifically
DROP FUNCTION IF EXISTS simple_get_authenticated_user(character varying) CASCADE;
DROP FUNCTION IF EXISTS simple_get_authenticated_user(VARCHAR) CASCADE;
DROP FUNCTION IF EXISTS simple_get_authenticated_user(VARCHAR(255)) CASCADE;

-- 3. Use a more aggressive approach - drop by specific_name
SELECT 'Using aggressive approach - drop by specific_name...' as step;
...
-- 4. Verify all functions are dropped
...
-- 5. Create functions with correct signatures

-- 5.1 Create simple_get_anonymous_user function
...
-- 5.2 Create simple_get_authenticated_user function
...
-- 5.3 Create increment_candidate_activity function
...
-- 5.4 Create simple_get_analytics function
...
SELECT simple_get_analytics('test_fix_1') as test_analytics;
...
WHERE cv.candidate_id = 'test_fix_1'
ORDER BY cv.created_at;

-- 8. Clean up test data
DELETE FROM candidate_views WHERE candidate_id = 'test_fix_1';

-- 9. Show final function list
...
SELECT '✅ Function return type error fixed!' as status;
SELECT '✅ All conflicting functions dropped' as cleanup_status;
SELECT '✅ Functions created with correct return types' as function_status;
SELECT '✅ All functions tested and working' as test_status;
